...
export default class HciCluster extends ProvCluster {
  ...
  async _pkgDetails() {
    ...
    const url = `/k8s/clusters/${ clusterId }/v1/harvester/uiinfo`;
    const uiInfo = await this.$dispatch('request', { url });

    // Work out the package Name
    const packageName = `${ HARVESTER_NAME }-${ uiInfo['ui-plugin-bundled-version'] }`;
    ...
    return {
      pkgUrl: 'https://yuzeng-dev.oss-cn-beijing.aliyuncs.com/harvester-0.1.0/harvester-0.1.0.umd.min.js',
      packageName
    };
  }
  ...
  async standaloneUrl() {
    ...
    // const url = `/k8s/clusters/${ row.status.clusterName }/v1/harvester/uiinfo`;
    // const uiInfo = await this.$store.dispatch('cluster/request', { url });
    // this.load(`harvester-${ uiInfo['ui-plugin-bundled-version'] }`, `/k8s/clusters/${ row.status.clusterName }/plugin-assets/harvester-0.1.0.umd.min.js`);
    // this.load(`harvester-${ uiInfo['ui-plugin-bundled-version'] }`, `https://yuzeng-dev.oss-cn-beijing.aliyuncs.com/harvester-1.13.0/harvester-1.13.0.umd.min.13.js`);
  ...
  // async goToHarvester(row) {
  //   const url = `/k8s/clusters/${ row.status.clusterName }/v1/harvester/uiinfo`;
  //   const uiInfo = await this.$store.dispatch('cluster/request', { url });

  //   // this.load(`harvester-${ uiInfo['ui-plugin-bundled-version'] }`, `/k8s/clusters/${ row.status.clusterName }/plugin-assets/harvester-0.1.0.umd.min.js`);
  //   this.load(`harvester-${ uiInfo['ui-plugin-bundled-version'] }`, `https://yuzeng-dev.oss-cn-beijing.aliyuncs.com/harvester-1.13.0/harvester-1.13.0.umd.min.13.js`);
  //   // console.log('----this.name', this.name, this.location);
  //   // ----this.name harvester-0.1.0 /pkg/harvester-0.1.0/harvester-0.1.0.umd.min.js
  // }
}
